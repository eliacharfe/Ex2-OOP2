...
// Prints welcome message
    ...
    int main()
    {
        ...
        auto nameField =          make_unique < Field < string >>  ("What is your name?                    ");
        auto idField =            make_unique < Field < uint32_t >>("What is your ID?                      ");
        auto yearOfBirthField =   make_unique < Field < int >>     ("What is your year of birth?           ");
        auto startDateField =     make_unique < Field < int >>     ("What is the date on which you wish to start your vacation? (ddmmyyyy)");      
        auto nightsNumField =     make_unique < Field < int >>     ("How many nights do you wish to invite?");
        auto pairRoomsField =     make_unique < Field < int >>     ("How many pair rooms do you want?      ");
        auto familyRoomsField =   make_unique < Field < int >>     ("How many family rooms do you want?    ");
        auto totalPeopleField =   make_unique < Field < int >>     ("How many people?                      ");    
        auto kidsUnder18Field =   make_unique < Field < int >>     ("Number of children under 18?          ");     
        auto adultsAbove18Field = make_unique < Field < int >>     ("Number of adults over 18?             ");

        // Creating the field validators
        auto nameValidator =            make_unique < NoDigitValidator >();
        auto idValidator =              make_unique < IDValidator >();
        auto ageValidator =             make_unique < RangeValidator          < int >>(currentYear() - MAX_AGE, currentYear() - MIN_AGE);  
        auto dateValidator =            make_unique < DateValidator           < int >>();        
        auto nightsValidator =          make_unique < NonNegativeValidator    < int >>();
        auto totalPairRoomsValidator =  make_unique < NotGreaterThanValidator < int >>(ROOMS_NUM);
        auto totalFamilyRoomsValidator= make_unique < NotGreaterThanValidator < int >>(ROOMS_NUM);
        auto totalPeopleValidator =     make_unique < NonNegativeValidator    < int >>();
        auto kidsUnder18Validator =     make_unique < NonNegativeValidator    < int >>();      
        auto adultsAbove18Validator =   make_unique < NonNegativeValidator    < int >>();
    ...
    void displayPrice(int totalPeople, int roomNum)
    {
        ...
        const auto priceStr = to_string(price);
        const auto spaces = string(23 - priceStr.size() / 2, ' ');
        cout << "+---------------------------------------------------+\n"
            "|                   The price is:                   |\n"
